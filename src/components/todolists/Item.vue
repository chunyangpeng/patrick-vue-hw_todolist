...
const emits = defineEmits(["remove-todolist"])

const removeToDoList = (e) => {
  const btn = e.currentTarget
  const id = btn?.dataset.todolistid
  if (id) {
    emits("remove-todolist", id)
  }
}
</script>
...
<template>
  <li class="flex items-center gap-2 text-2xl">
    <button :data-todolistid="todolist.id" @click="removeToDoList" class="btn btn-xs">
      <svg
    ...
    <router-link :to="{ name: 'detail', params: { auo: todolist.id } }">
      {{ todolist.title }}
    </router-link>
  </li>
</template>
